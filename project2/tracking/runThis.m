...
pIdx = find(startXs>7 & startXs<(w-7));
startXs = startXs(pIdx);
startYs = startYs(pIdx);

pIdy = find(startYs>7 & startYs<(h-7));
startXs = startXs(pIdy);
startYs = startYs(pIdy);
...
for j =1:50
   ...
   [Xs Ys active] = predictTranslationAll(pointXs(:,j),pointYs(:,j),im1,im2);
